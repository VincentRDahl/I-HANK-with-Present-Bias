...
class IHANKModelClass(EconModelClass,GEModelClass):
    ...
    def settings(self):
        ...
        # c. GE
        self.shocks = ['ZTH','ZNT','M_s','rF','PF_s','beta','G','i_shock', 'CTH_s'] # exogenous inputs
    ...
    def setup(self):
        ...
        # a. discrete states
        par.Nsector = 2 # Number of sectors
        par.Ndelta = 2 # Number of delta states
        par.Nz = 7 # idiosyncratic productivity
        par.sT = 0.29 # share of workers in tradeable sector
        par.sdelta = 0.25 # share of present biased households

        # b. preferences
        par.beta = 0.972 # discount factor
        par.delta = 0.5 # present bias
        ...
        par.etaT = 1.5 # elasticity of substitution between tradeable and non-tradeable goods
        
        par.alphaF = 0.35 # share of foreign goods in home tradeable consumption
        par.etaF = 1.81 # elasticity of substitution between home and foreign tradeable goods
        ...
        par.jump_CTH_s = 0.00 # initial jump
        par.rho_CTH_s = 0.00 # AR(1) coefficeint
        par.std_CTH_s = 0.00 # std.
    ...
    def allocate(self):
        ...
        par = self.par
        par.Nfix = par.Nsector * par.Ndelta # number of sectors sectors and delta states
        par.delta_grid = np.zeros(par.Nfix) # grid for delta states
        par.sector_grid = np.zeros(par.Nfix)
        self.allocate_GE()
