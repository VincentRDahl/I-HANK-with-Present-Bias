...
def prepare_hh_ss(model):
    ...
    # d. delta, present bias
    delta_grid = np.linspace(1.0,par.delta,par.Ndelta)
    par.delta_grid[:] = np.tile(delta_grid, par.Nsector)

    # e. sector
    sector_grid = np.array([0,1])
    par.sector_grid[:] = np.repeat(sector_grid,par.Ndelta)
    ...
    for i_fix in range(par.Nfix):
        if i_fix == 0:
            ...
            ss.Dbeg[i_fix,:,0] = e_ergodic*par.sT*par.sdelta
        elif i_fix == 2:
            ss.Dbeg[i_fix,:,0] = e_ergodic*(1-par.sT)*(1-par.sdelta)
        elif i_fix == 3:
            ss.Dbeg[i_fix,:,0] = e_ergodic*(1-par.sT)*par.sdelta
        else:
    ...
    for i_fix in range(par.Nfix):
        for i_z in range(par.Nz):
            ...
            if i_fix == 0 or i_fix == 1:
                inc = ss.inc_TH/par.sT*z
            elif i_fix == 2 or i_fix == 3:
